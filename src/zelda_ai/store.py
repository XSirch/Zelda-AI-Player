    ...
    def learn_trajectory(self, namespace: str, origin: dict, destination: dict,
        ...
        for action in actions[:96]:
            ...
            if skill not in {"move", "turn", "interact", "wait", "camera_center",
                              ...
                              "interact_with_actor", "traverse", "traverse_to", "traverse_exit"} or not isinstance(args, dict):
                return None
        ...
        if not safe_actions or not any(a["skill"] in {
                "move", "turn", "navigate_to", "traverse", "traverse_to", "traverse_exit"}
