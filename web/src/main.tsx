...
function ProgressPanel({ game, checkpoint }: { game: GameState | null; checkpoint: Snapshot['checkpoint_plan'] }) {
  ...
  return <>
    {checkpoint && <section className="panel progress-panel">
      <div className="section-head"><span>PLANO / CHECKPOINT</span><span className="muted">{checkpoint.completed_count} / {checkpoint.total}</span></div>
      {current ? <div className="progress-grid">
      ...
      {tab === 'AO VIVO' && <><MetricStrip metrics={snap?.metrics ?? null}/><div className="cockpit"><div className="live-column">
        ...
        <div className="live-tab-stage">
          ...
          {liveTab === 'ESTADO' && <div className="telemetry panel">
            ...
            <div><span>AUTOSAVE</span><strong>{game?.autosave ? (game.autosave.pending ? `Pendente · SCENE ${game.autosave.target_scene}` : `${game.autosave.count} saves · última SCENE ${game.autosave.last_scene >= 0 ? game.autosave.last_scene : '—'}`) : '—'}</strong></div>
          ...
          {liveTab === 'DECISÃO' && <>
            <section className="panel"><div className="section-head">DECISÃO E RESULTADO</div><div className="decision"><span className="eyebrow">{snap?.last_decision?.skill ?? 'SEM AÇÃO'}</span><h3>{snap?.last_decision?.goal ?? 'Pronto para uma nova execução'}</h3><p>{snap?.last_decision?.summary ?? 'O modelo recebe estado estruturado, memória de mundo e perfis aprendidos de inimigos.'}</p>{snap?.last_result && <code>{snap.last_result.status} / {snap.last_result.reason}</code>}{snap?.last_result?.auto_navpath && <p className="muted">AUTO NAVPATH → {snap.last_result.selected_affordance?.kind?.replaceAll('_', ' ').toUpperCase() ?? snap.last_result.controller_skill ?? 'TRAVERSAL'}</p>}</div></section>
