...
Dados expostos: posição/orientação/colisão/água, vida/magia/rupias, inventário/equipamento/progresso, câmera, cena/sala/entrance/dia-noite, diálogo, ação contextual, target, `room_actors` (até 64, inclusive off-camera), `nearby_actors` como subconjunto renderizado e telemetria de travessia do player (`wall_flags`, ladder/ledge/can_climb/can_down). `navigation_probes` faz 16 raycasts de piso locais (8 direções × 70/140 unidades) e publica `delta_y`, permitindo reconhecer escadas, plataformas e descidas sem depender de atores. Não há navmesh global, escrita direta em estado de jogo nem eventChkInf/flags ocultos de roteiro deliberadamente expostos.
