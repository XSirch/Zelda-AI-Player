...
- Percepção estruturada: cena nomeada, room/entrance, dia/noite, pose/colisão/água, diálogo decodificado, ação contextual associada ao ator quando disponível, target, `room_actors` com todos os atores ativos da sala atual (inclusive fora da câmera) e `nearby_actors` como subconjunto renderizado, inventário com nomes/munição aplicável, equipamento/progresso visível no pause, cutscene, game-over e ocarina. Screenshots continuam fora do prompt normal.
